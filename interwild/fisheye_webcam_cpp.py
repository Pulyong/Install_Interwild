...
def visualize(fisheye,original_img,skeleton,mesh):
    res_img = original_img.copy()
    res_img = np.concatenate((fisheye,res_img,skeleton,mesh), axis=1)
    return res_img
...
# snapshot load
model_path = '/home/shu/Desktop/Yongjin/hands/git_models/InterWild/demo/snapshot_6_ego.pth'
...
while(True):
    ...
    retval, original_bgr = webcam.read()
    ...
    original_bgr = original_bgr[:,:w//2,:]
    fisheye = original_bgr[:,:w//2,:].copy()

    original_bgr = undistortImage(original_bgr)

    original_img = original_bgr[:,:,::-1].copy()
    ...
    res_img = visualize(fisheye/255,vis_box/255, vis_skeleton/255, render_out[0].cpu().numpy()/255)
    ImShow(res_img)
    ...
    if args.save_frame:
        ...
        cv2.imwrite(osp.join(render_save_path, file_name + '_total.jpg'), np.concatenate((fisheye,vis_box,vis_skeleton,render_out[0].cpu().numpy()), axis=1))
